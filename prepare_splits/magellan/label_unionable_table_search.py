...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate unionable table search labels from derived tables.")
    ...
    for row in df.itertuples(index=False):
        ...
        derived = str(row.derived_table)
        dataset_name = str(row.dataset_name)
        unionable_id = extract_prefix(origin)

        records.append(
            {"table_name": origin, "dataset_name": dataset_name, "unionable_id": unionable_id}
        )
        records.append(
            {"table_name": derived, "dataset_name": dataset_name, "unionable_id": unionable_id}
        )

    LABEL_DIR.mkdir(parents=True, exist_ok=True)
    out_path = LABEL_DIR / "unionable_table_search_labels.csv"
    pd.DataFrame(records, columns=["table_name", "dataset_name", "unionable_id"]).to_csv(out_path, index=False)
